...
DEV_UBOONECODE_FOLDER=v08_00_00_40a_dl
INPUTLIST=${WORKDIR}/corsika_bnb_nu_pi0.runlist
OUTPUT_DIR=/cluster/tufts/wongjiradlab/larbys/data/ub_on_tufts/bnb_nu_pi0_corsika/
GEN_FCL=prodgenie_bnb_nu_pi0filter_cosmic_uboone_on_tufts_wbaseseedtemplate.fcl
...
cd $WORKDIR/dev/${DEV_UBOONECODE_FOLDER}
...
do
    ...
    job_fcl=$(printf "prodgenie_bnb_nu_pi0_cosmic_uboone_on_tufts_file%d.fcl" ${fileno})
    JOBGENSEED=$((10000 + ${fileno}))
    sed "s/__TEMPLATE_SEED_NJOB__/${JOBGENSEED}/g" $GEN_FCL > $job_fcl
    echo "" >> ${job_fcl}
    ...
    lar -c $job_fcl -n $NENTRIES > log_gen.txt
    head -n 100 log_gen.txt >> $local_logfile
    tail -n 100 log_gen.txt >> $local_logfile
    rm log_gen.txt
    ...
    lar -c wirecell_g4_uboone.fcl -s $genfile -n $NENTRIES > log_g4.txt
    head -n 100 log_g4.txt >> $local_logfile
    tail -n 100 log_g4.txt >> $local_logfile
    rm log_g4.txt
    rm $genfile
    ...
    lar -c wirecell_detsim_uboone.fcl -s $g4file -n $NENTRIES > log_detsim.txt
    head -n 100 log_detsim.txt >> $local_logfile
    tail -n 100 log_detsim.txt >> $local_logfile
    rm log_detsim.txt
    rm $g4file
    ...
    lar -c reco_uboone_mcc9_8_driver_stage1.fcl -s $detsimfile -n $NENTRIES > log_reco1.txt
    head -n 100 log_reco1.txt >> $local_logfile
    tail -n 100 log_reco1.txt >> $local_logfile
    rm log_reco1.txt
    rm $detsimfile
    ...
    lar -c standard_larcv_uboone_mctruth_prod_on_tufts.fcl -s $reco1file -n $NENTRIES > log_larcv.txt
    head -n 100 log_larcv.txt >> $local_logfile
    tail -n 100 log_larcv.txt >> $local_logfile
    rm log_larcv.txt
